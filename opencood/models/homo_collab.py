...
# Adapted for HEAL framework - removed unsupported dependencies
...
# Note: BevSegHead and NaiveDecoder not available in HEAL, removed for compatibility
...
class HomoCollab(nn.Module):
    def __init__(self, args):
        ...
        if self.head_method == "point_pillar_pyramid_object_detection_head":
            ...
            # BevSegHead not available in HEAL framework
            raise NotImplementedError(f"Head method {self.head_method} not supported in HEAL framework.")
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        if modality_name in modality_count_dict:
            ...
            if self.multi_sensor:
                feature_camera = eval(f"self.encoder_{modality_name}_camera")(data_dict, modality_name)
                """
                ...
                feature_lidar = eval(f"self.encoder_{modality_name}_lidar")(data_dict, modality_name)
